...
/// Fetch the most recent confirmed anchor batch that includes an event for
/// `product_id`. Pending batches are excluded.
#[instrument(skip(db), err)]
pub async fn latest_for_product(
    ...
    sqlx::query_as::<_, AnchorBatch>(
        r#"
        SELECT b.id, b.records_hash, b.tx_hash, b.block_number, b.status, b.anchored_at
        FROM anchor_batches b
        JOIN provenance_events e ON e.anchor_batch_id = b.id
